...
pub const ACTIONS: &[ActionInfo] = &[
    ...
    ActionInfo {
        id: Some(ActionId::OpenHealth),
        scope: Scope::Normal,
        section: Section::Main,
        keys: "`w`",
        description: "Why is it slow? Open the Health tab for the selected torrent",
        short: Some("Why is it slow? (Health tab)"),
        // Not in the hint line, like Detach: the Normal line already clips
        // on narrow terminals. The stall marker in the Status column and the
        // Health line on the Stats tab are the in-context pointers; the
        // palette, `?` and the README carry the key.
        hint: None,
        hint_when_empty: false,
        in_palette: true,
        available: has_visible_torrents,
    ...
    ActionInfo {
        ...
        keys: "`Tab`",
        description: "Cycle tabs (Stats → Info → Files → Peers → Health)",
    ...
    ActionInfo {
        ...
        section: Section::Detail,
        keys: "`j` / `k`",
        description: "Navigate files (Files tab) or peers (Peers tab); scroll the Health tab",
        short: Some("Navigate files/peers, scroll Health"),
        hint: Some("j/k:navigate"),
        hint_when_empty: false,
        in_palette: false,
        available: always,
